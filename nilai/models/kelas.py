...
class kelas(models.Model):
    _name = 'nilai.kelas'
    _description =  'class untuk menyimpan data kelas yang dibuka suatu semester'

    name = fields.Char(compute="_compute_name", store=True, recursive=True)
    semester = fields.Selection([('Genap', 'Genap'),
                                 ('Gasal', 'Gasal')], states={'draft':[('readonly', False)]}, )
    tahun = fields.Char("Tahun", size=15, defaultp="2021/2022", required=True, readonly=True,
    ...
    line_ids = fields.One2many('nilai.kelas.lines', 'kelas_id', string='Nilai', readonly=True,
                               states={'draft': [('readonly', False)]})
    _sql_constraints = [('name_unik', 'unique(mk_id, semester, tahun)',
                         _("The Class is Already Exist for the Semester"))]

    @api.depends('mk_id.name','semester','tahun')
    def _compute_name(self):
        for a in self:
            a.name = '%a - %a - %a' % (a.mk_id.name, a.semester, a.tahun)
...
class kelas_lines(models.Model):
    ...
    grade = fields.Selection([('A', 'A'),
                              ...
                              ('D', 'D'),
                              ('E', 'E')])
    _sql_constraints = [('name_unik', 'unique(kelas_id, mhs_id)',
                         _("The Student is Already Exist!"))]